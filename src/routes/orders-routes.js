...
import {
  createOrder,
  findLinesInCart,
  findOrderById, getMenuItemById, listOrders,
  seeOrderState,
  setOrderState
} from '../lib/db.js';
...
async function updateOrder(req, res) {
  const { orderid } = req.params;
  console.log(orderid);
  const oldorder = await seeOrderState(orderid);
...
ordersRouter.get('/', jwtPassport.authenticate('jwt', { session: false }),
  ensureIsAdmin, allOrdersValidation, allOrdersXss,
